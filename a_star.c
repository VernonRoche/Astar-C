...
node createNode(position pos, double cost, struct node* parent, double score){
...
int compareNodes(const void *x, const void *y){
  node first_node= *(node*)x;
  node second_node= *(node*)y;
  double result=(first_node->score)-(second_node->score);
  if (result<0) {
    return -1;
  }
  if (result==0) {
...
int compareNodesV2(node n1, node n2){
...
int comparePosition(position * p1, position * p2){
...
int getHeapMaxSize(int dim_X, int dim_Y){
    return ((dim_X-2)*(dim_Y-2)*8) + (4*3) + ( ((dim_X-2)*5)*2 +((dim_Y-2)*5)*2);
}
...
double A_star(grid G, heuristic h){
  ...
  // s'échapper de la grille.
    heap P = heap_create((G.X-2) * (G.Y-2),compareNodesV2);
    heap Q = heap_create(G.X * G.Y,compareNodesV2);

    node s = malloc(sizeof(*s));
    s->cost = 0;
    s->parent = NULL;
    s->score = s->cost + h(G.start,G.end,&G);
    s->pos = G.start;
    heap_add(Q,s);
    ...
    while(!heap_empty(Q)){
        ...
        node u = heap_pop(Q);
        if(comparePosition(&u->pos, &G.end) == 0){
            //Return path
            node parent = u;
            double scoreTotal = 0;
        ...
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                ...
                v->pos.x = u->pos.x+i;
                v->pos.y = u->pos.y+j;
                if(G.value[v->pos.x][v->pos.y] == V_WALL){
                    continue;
                }
                v->cost = weight[G.value[v->pos.x][v->pos.y]] + u->cost;
                v->score = v->cost + h(v->pos, G.end, &G);
                v->parent = u;

                if(G.mark[v->pos.x][v->pos.y] != M_USED){
                    if(G.mark[v->pos.x][v->pos.y] != M_FRONT){
                        heap_add(Q, v);
                        G.mark[v->pos.x][v->pos.y] = M_FRONT;
                    }else if(v->cost >= weight[G.value[v->pos.x][v->pos.y]]){
                        free(v);
                    }
                }
            }
