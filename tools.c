...
#define COLOR_GROUND   0.0, 0.0, 0.0  // couleur du fond
#define COLOR_POINT    1.0, 0.0, 0.0  // point
#define COLOR_LINE     1.0, 1.0, 1.0  // ligne de la tournée
#define COLOR_PATH     0.0, 1.0, 0.0  // chemin
#define COLOR_ROOT     0.9, 0.8, 0.3  // racine, point de départ
#define COLOR_TREE     0.0, 0.4, 0.0  // arête de l'arbre
...
void drawX(point *V, int n, int *P, int k, graph *G) {
  ...
  if (V && P && (P[0]>=0) && ((G && (mst&2)) || (!G && (mst&1)))) {
    if(k>n) glColor3f(COLOR_LINE);
    else glColor3f(COLOR_PATH);
    for (int i = 0; i < k - 1; i++){
      if(oriented)
